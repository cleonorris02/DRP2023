...
student = (name = "Cleo Norris", kerberos_id = "cleo")
...
function clean(text)
	cleaned_string = filter(isinalphabet, lowercase(unaccent(text)))
end
...
# ╔═╡ 80fc301d-c586-4804-9f33-8cd2ae0ed92a
# question: was there a more efficient way to answer this question than just looking at the outputted list?
...
# pseudocode
# for i in length(sample_freq_matrix)
	# if sample_freq_matrix[index_of_letter('i'), index_of_letter('i')] > 0.0
		# double_letters_list += sample_freq_matrix[index_of_letter('i'), index_of_letter('i')]

double_letters = ['e', 'l']
# want to do this in a more efficient way
...
# method: find the max in the 'w' column in the sample freq matrix
...
row_col_answer = md"""
The column sums give the frequency with which each letter appears after some other letter. The row sums give the total freq with which each letter appears before another letter. (i.e. starting or ending a letter combo)
"""
...
# what does it mean by summing the differences?
